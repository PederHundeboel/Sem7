...
def read_hbase(con, table_name, key, cf='hb'):
    ...
    columns = {col.split(':')[1] : value for col, value in list(column_dtype.items())}
    ...
    row_start = str(key + 'rows').encode() + struct.pack('>q', 0)
    row_end = str(key + 'rows').encode() + struct.pack('>q', sys.maxsize)
    rows = table.scan(row_start=row_start, row_stop=row_end)
    df = pd.DataFrame(columns=columns)
    for row in rows:
        df_row = {key.split(':')[1]: value for key, value in list(row[1].items())}
        df = df.append(df_row, ignore_index=True)
        print()
    for column, data_type in list(columns.items()):
